...
@section('content')
    ...
    <style>
        ...
        /* ═══════════════════════════════════════════
                                        ...
                                        ═══════════════════════════════════════════ */
        .reg-hero {
            background: linear-gradient(135deg, #043AA1 0%, #1a6b30 45%, #0059ff 100%);
            border-radius: 20px;
                        ...
                        <div class="row">
                            <div class="col-md-3 mb-3">
                                ...
                                <select name="category" id="category" class="reg-select select2" required>
                                    ...
                                    @include('partials.examination-level-options', ['show_group' => false])
                                </select>
                            </div>
                            <div class="col-md-3 mb-3">
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content" style="border-radius:16px; overflow:hidden; border:none;">
                <div class="modal-header"
                    style="background: linear-gradient(135deg, #043AA1, #0059ff); color: white; border:none; padding:20px 28px;">
        ...
        <script>
            $(document).ready(function () {
                ...
                $(document).on('click', '.delete-student', function () {
                    ...
                    Swal.fire({
                        ...
                        $.ajax({
                            ...
                            success: r => Swal.fire({ icon: 'success', title: 'Deleted!', html: `Student <b>${studentId}</b> removed.`, confirmButtonColor: '#0059ff' }).then(() => location.reload()),
                ...
                $('#schoolRegistrationForm').on('submit', function (e) {
                    ...
                    if (!valid) { Swal.fire({ icon: 'error', title: 'Missing Fields', text: 'Please fill in all required fields', confirmButtonColor: '#0059ff' }); return; }
                    ...
                    Swal.fire({
                        ...
                        icon: 'question',
                        showCancelButton: true,
                        confirmButtonColor: '#0059ff', cancelButtonColor: '#dc2626',
                        ...
                        $.ajax({
                            ...
                            success: r => Swal.fire({ icon: 'success', title: 'Success!', text: r.message, confirmButtonColor: '#0059ff' }).then(res => { if (res.isConfirmed) location.reload(); }),
                            error: xhr => Swal.fire({ icon: 'error', title: 'Error', text: xhr.responseJSON?.message || 'Something went wrong', confirmButtonColor: '#0059ff' })
                        });
                    });
                });
                ...
                $('#edit_photo_input').on('change', function (e) {
                    ...
                    if (!['image/jpeg', 'image/jpg', 'image/png'].includes(file.type)) {
                        Swal.fire({ icon: 'error', title: 'Invalid File', text: 'Please select JPG or PNG.', confirmButtonColor: '#0059ff' });
                        $(this).val(''); return;
                    }
                    if (file.size > 2 * 1024 * 1024) {
                        Swal.fire({ icon: 'error', title: 'Too Large', text: 'Max 2MB.', confirmButtonColor: '#0059ff' });
                        $(this).val(''); return;
                    }
                ...
                $('#remove_photo_btn').on('click', function () {
                    ...
                    Swal.fire({ title: 'Remove Photo?', text: "Are you sure?", icon: 'warning', showCancelButton: true, confirmButtonColor: '#dc2626', cancelButtonColor: '#0059ff', confirmButtonText: 'Yes, remove!' })
                        .then(result => {
                            if (!result.isConfirmed) return;
                            ...
                            $.ajax({
                                ...
                                success: r => {
                                    ...
                                    setTimeout(() => $('#upload_status').fadeOut(), 3000);
                                    currentPhotoFile = null;
                                    Swal.fire({ icon: 'success', title: 'Removed!', text: r.message, confirmButtonColor: '#0059ff' });
                                },
                ...
                if (editDrop) {
                    ...
                    ['dragenter', 'dragover'].forEach(ev => editDrop.addEventListener(ev, () => { editDrop.style.borderColor = '#0059ff'; editDrop.style.background = '#f0f9f0'; }, false));
                ...
                $('#editStudentForm').on('submit', async function (e) {
                    ...
                    if (!fd.category || !fd.admission_year || !fd.student_name || !fd.student_sex) {
                        Swal.fire({ icon: 'error', title: 'Missing Fields', text: 'Please fill in all required fields', confirmButtonColor: '#0059ff' }); return;
                    }
                    const confirm = await Swal.fire({ title: 'Update Student?', text: 'Confirm update?', icon: 'question', showCancelButton: true, confirmButtonColor: '#0059ff', cancelButtonColor: '#dc2626', confirmButtonText: 'Yes, update!' });
                    ...
                    try {
                        ...
                        Swal.fire({ icon: 'success', title: 'Updated!', text: res.message, confirmButtonColor: '#0059ff' })
                ...
                $('#step3_filter_btn').on('click', function () {
                    ...
                    if (!year || !category) {
                        Swal.fire({ icon: 'warning', title: 'Required', text: 'Please select both Year and Category.', confirmButtonColor: '#0059ff' }); return;
                    }
                ...
                $('#step3_submit_btn').on('click', function () {
                    ...
                    if (checked.length === 0) {
                        Swal.fire({ icon: 'warning', title: 'None Selected', text: 'Select at least one student.', confirmButtonColor: '#0059ff' });
                        return;
                    }
